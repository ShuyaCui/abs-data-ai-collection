...
补齐《发行说明书》p102–104 的确定性预测回收切片，并消除代码审查发现的三处重复逻辑。范围仅限字段 15、36、37、38 与对应的批处理路径。
...
- 仅接受文件名包含“发行说明书”、产品粒度实体、物理页 102–104 且 `ocr_requested=False` 的原生 block；不调用模型或外部服务。
...
- 字段 15 仅从 p104 的“全部未偿贷款本息费总额”及唯一金额/单位产生，万元转换为 CNY。
- 字段 38 从明确出现的机构名按文档顺序去重，输出 `string[]`；它不依赖采用关系。

### 批处理与来源优先级

- `extract-folder` 为发行说明书新增 p102–104 的 `pypdf` 解析范围。
- 批处理先处理评级报告，再处理发行说明书；若已存在评级报告来源的字段 15，则丢弃本切片生成的字段 15 候选。字段 36/37 当前不改变现有来源选择机制。
- 单文件 `extract` 仍可输出发行说明书字段 15 候选；跨文档优先级只由 folder batch 执行。
...
- 测试覆盖成功、缺采用关系、冲突中债值、OCR 页/页码边界拒绝、字段 38 无采用关系、字段 15 的批处理来源优先级。
- 每个输出断言产品粒度、`disclosed` 状态、单位、完整 parser-owned evidence。
- 现有离线回归套件保持通过。
